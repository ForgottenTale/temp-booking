...
module.exports = {
    ...
    getAllUsers: function(constraint, done){
		...
		if(constraint.user.activeOu.id==1 && constraint.user.activeOu.admin==1){
			query = "SELECT *, person.name AS name, user._id as id, ou.name as ouName FROM user INNER JOIN person ON person_id=person._id INNER JOIN ou_map ON ou_map.person_id=person._id INNER JOIN ou ON ou._id=ou_map.ou_id ";
    ...
    getActivity: async function(userId, ouId, done){
		let returnData = {};
		try{
			let query = "SELECT status, count(*) FROM blt";
            // if(ouId != 1)
            //     query += " WHERE ou_id=" + ouId ;
            query+= ` WHERE creator_id=${userId} AND ou_id=${ouId} GROUP BY status;`;
