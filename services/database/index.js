			...
			if(input.encourages){
				...
				if(info.level==0){
					...
					sameSlotBookings.forEach(async booking=>{
						...
						let creator = await executeQuery(`SELECT * FROM blt_id INNER JOIN user ON user._id=creator_id INNER JOIN person ON person_id=person._id WHERE _id=${booking._id}`);
